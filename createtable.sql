...
create policy "ブログは誰ても参照可能" on blogs for select using ( true );
create policy "自信のブログを追加" on blogs for insert with check (auth.uid() = user_id);
...
create policy "プロフィールは誰ても参照可能" on profiles for select using ( true );
